...
def load_config(path):
    ...
    if not config.seed_data_glob:
        raise ValueError('seed_data_glob must be provided for lead optimization training')
    return config
...
def resolve_output_dir(config, config_path):
    ...
    if os.path.isdir(cluster_root):
        base_dir = os.path.join(cluster_root, 'runs', 'cpgrpo_leadopt')
    else:
        repo_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.realpath(__file__)))))
        base_dir = os.path.join(repo_root, 'runs', 'cpgrpo_leadopt')
...
class LeadOptCpGRPOTrainer:
    def __init__(self, config, output_dir):
        ...
        self.policy = LeadOptCpGRPOPolicy(
        ...
        self.reward_model = LeadOptimizationReward(sim_weight=config.sim_weight)
        ...
        if config.report_to:
            ...
            self.accelerator.init_trackers('genmol-cpgrpo-leadopt', config=asdict(config), init_kwargs=init_kwargs or None)
    ...
    def _record_reward_metrics(self, mode, metadata):
        ...
        self._last_reward_metrics[mode] = {
            ...
            'rewards/sim_mean': float(metadata['rewards/sim_mean']),
        }
        ...
        bucket['rewards/sim_mean'].append(float(metadata['rewards/sim_mean']))
    ...
    def _generate_and_score_completions(self, mode):
        ...
        reward_records = self.reward_model.score(rollout.seed_smiles, rollout.smiles)
        ...
        local_sim = torch.tensor(
            [float('nan') if record.sim is None else float(record.sim) for record in reward_records],
            device=self.device,
        )
        ...
        gathered_sim = self.accelerator.gather(local_sim)
        ...
        expanded_ids = rollout.input_ids.unsqueeze(0).expand(self.config.num_iterations, -1, -1)
            ...
            log_rows.append(
                {
                    ...
                    'spec': asdict(spec),
                    'seed_smiles': record.seed_smiles,
                    'safe': safe_string,
        ...
        metadata = {
            ...
            'rewards/sim_mean': _nanmean(gathered_sim),
        }
    ...
    def _consume_logged_metrics(self, mode, step, buffer_cycle):
        ...
        metrics = {
            ...
            'rewards/sim_mean': _reward_metric('rewards/sim_mean'),
    ...
    def _log_metrics(self, split, metrics):
        if self.accelerator.is_main_process:
            logger.info(
                ...
                'rewards/soft_mean=%.6f rewards/sim_mean=%.6f grad_norm=%.6f lr=%.8f%s',
                split,
                metrics['step'],
                metrics['reward_mean'],
                metrics['reward_std'],
